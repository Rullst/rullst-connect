...
async fn test_github_get_user_success() {
    ...
    assert_eq!(secrecy::ExposeSecret::expose_secret(&user.access_token), "mock_access_token_123");
    assert_eq!(
        user.refresh_token.as_ref().map(|s| secrecy::ExposeSecret::expose_secret(s).as_str()),
